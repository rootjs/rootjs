...
std::map<std::string, MemberProxyInitializator>
    ObjectProxyFactory::memberProxyMap;
std::map<std::string, GlobalProxyInitializator>
    ObjectProxyFactory::globalProxyMap;

void ObjectProxyFactory::traverseClass(TClassRef &classRef,
                                       ObjectProxy &proxy) {
  ...
  while ((member = (TDataMember *)nextProperty())) {
    ...
    ObjectProxy *memberProxy =
        ObjectProxyFactory::createObjectProxy(*member, classRef, proxy);
    nodeObject->Set(
        v8::String::NewFromUtf8(v8::Isolate::GetCurrent(), member->GetName()),
        memberProxy->get());
  }
}
...
ObjectProxy *ObjectProxyFactory::createObjectProxy(TGlobal &object) {
  ...
  ObjectProxy *nonObjectProxy =
      determineProxy((TObject *)object.GetAddress(), object, TClassRef());
...
ObjectProxy *ObjectProxyFactory::createObjectProxy(const TDataMember &type,
                                                   TClassRef scope,
                                                   ObjectProxy &holder) {
  /*
...
ObjectProxy *ObjectProxyFactory::determineProxy(const TDataMember &type,
                                                TClassRef ref) {
...
ObjectProxy *ObjectProxyFactory::determineProxy(void *address,
                                                const TGlobal &type,
                                                TClassRef ref) {
  std::string typeString = std::string(type.GetTypeName());
